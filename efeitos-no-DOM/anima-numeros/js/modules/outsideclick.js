...
export default function outsideClick(element, events, callback) {
  ...
  // é verificado se o elemento tem o atributo 'data-outside', para evitar adicionar vários listeners iguais para o HTML
  ...
  if(!element.hasAttribute(outside)){
    events.forEach((event) => {
      ...
      // add os eventos ao elemento html e guarda na memória o callback

      // ATENÇÃO -> É usado a função setTimeout para que o addEventListener seja executado somente e extritamente somente depois que todo o código JS for executado, assim evitando que, caso o click para exibir um elemento for fora da area já inicial, evite que a função do listener já seja executada.
    })
    element.setAttribute(outside, ''); // é adicionado ao elemento que chamou a função o atributo data-outside, para a verificação do if anterior
  }
