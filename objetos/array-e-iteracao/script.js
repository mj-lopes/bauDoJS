...
// [].MAP() - sempre que quiser fazer o retorno de um array use map!
...
// [].REDUCE() - BOM PARA EXTRAÇÃO DE DADOS ESPECIFICOS PARA UM NOVO ITEM, SEJA ARRAY, OBJETO, NUMBER, STRING ...
...

{
  ...
  const frutasRight = frutas.reduceRight(
    (acomulador, fruta) => acomulador + " " + fruta
  );
  const frutasLeft = frutas.reduce(
    (acomulador, fruta) => acomulador + " " + fruta
  );
...

{
  ...
  const totalCompras = compras.reduce((acomulador, compra) => {
    const valorLimpo = +compra.preco.replace("R$ ", "").replace(",", ".");
    return acomulador + valorLimpo;
  }, 0);

  console.log("R$: ", totalCompras);
}
